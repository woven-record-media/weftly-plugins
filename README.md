...
| `/weftly-editing:write-social-posts <words.json, .srt, or .txt>` | Generate ready-to-paste social media posts for LinkedIn, Reddit, Instagram, X, and Bluesky from a transcript in a chosen tone (professional, conversational, bold, or inspirational). 2 A/B variants per platform, platform-tuned char limits and hashtag norms, reusable pull-hooks. Writes one `<base>_social_<platform>.md` file per selected platform. Read-only and free. |
| `/weftly-editing:transcribe-and-write-social-posts <file>` | Bundled: transcribe → write-social-posts in one shot. |
